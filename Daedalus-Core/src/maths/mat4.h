...
namespace daedalus { namespace maths {
	...
	struct Mat4
	{
		...
		inline Mat4& invert()
		{
			...
			inverse.multiply(oneOverDeterminant);
			this->columns[0] = inverse.columns[0];
			this->columns[1] = inverse.columns[1];
			this->columns[2] = inverse.columns[2];
			this->columns[3] = inverse.columns[3];
		...
		static inline bool decomposeTransform(const Mat4& transform, Vec3& outPosition, Vec3& outRotation, Vec3& outScale)
		{
			...
			outScale.x = Vec3::length(Row[0]);
			Row[0] = Vec3::scale(Row[0], 1.0f);
			outScale.y = Vec3::length(Row[1]);
			Row[1] = Vec3::scale(Row[1], 1.0f);
			outScale.z = Vec3::length(Row[2]);
			Row[2] = Vec3::scale(Row[2], 1.0f);
